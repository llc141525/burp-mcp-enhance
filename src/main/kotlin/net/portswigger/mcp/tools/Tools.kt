...
fun Server.registerTools(api: MontoyaApi, config: McpConfig) {
    ...
    if (api.burpSuite().version().edition() == BurpSuiteEdition.PROFESSIONAL) {
        mcpPaginatedTool<GetScannerIssues>("Displays information about issues identified by the scanner. Use count ≤ 20 per request to avoid truncation.") {
    ...
    mcpPaginatedTool<GetProxyHttpHistory>("Displays items within the proxy HTTP history. Use count ≤ 20 per request to avoid truncation.") {
        val allowed = runBlocking {
    ...
    mcpPaginatedTool<GetProxyHttpHistoryRegex>("Displays items matching a specified regex within the proxy HTTP history. Use count ≤ 20 per request to avoid truncation.") {
        val allowed = runBlocking {
            checkHistoryPermissionOrDeny(HistoryAccessType.HTTP_HISTORY, config, api, "HTTP history")
        }
        if (!allowed) {
    ...
    mcpPaginatedTool<GetProxyWebsocketHistory>("Displays items within the proxy WebSocket history. Use count ≤ 20 per request to avoid truncation.") {
        val allowed = runBlocking {
    ...
    mcpPaginatedTool<GetProxyWebsocketHistoryRegex>("Displays items matching a specified regex within the proxy WebSocket history. Use count ≤ 20 per request to avoid truncation.") {
        val allowed = runBlocking {
            checkHistoryPermissionOrDeny(HistoryAccessType.WEBSOCKET_HISTORY, config, api, "WebSocket history")
        }
        if (!allowed) {
